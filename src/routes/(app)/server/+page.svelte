...
<div class="content">
	<div class="inner">
        ...
        already using another."
					...
					<Setting
						title="TVDB Key"
						desc="Provide your own TVDB API Key (Required for Emby auto-scrobbling)"
					>
						<input
							type="password"
					...
					<Setting
						...
						desc="Requests per second used for external APIs. 0 or unset falls back to 10 requests per second. CAUTION: Setting this higher than 40 will likely cause TVDB and TMDB to ban/throttle your API key."
					>
						<input
							type="number"
							min="1"
							...
							onblur={() => {
								...
								updateServerConfig(
									...
									Number(serverConfig!.GLOBAL_RATE_LIMIT_RPS),
									() => {
					...
					<Setting
						title="Default Emby Rating"
						desc="Rating saved for auto-imported episodes (0-10) when using Emby (You can edit each rating after it's been imported)."
					>
						<input
							type="number"
							min="0"
							...
							onblur={() => {
								...
								updateServerConfig(
									"EMBY_DEFAULT_RATING",
									Number(serverConfig!.EMBY_DEFAULT_RATING),
									() => {
