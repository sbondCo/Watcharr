<script lang="ts">
	...
	async function approve(r: ArrRequestResponse) {
		...
		if (r.content.type === "tv") {
			...
			notify({
				type: "error",
				text: "Unknown content type, can't continue approval!",
			});
			return;
		}
						...
						<div class="wordsnstuff">
							...
							<div class="btns">
								{#if r.username}
									...
									>
										By {r.username} &#160;&#8226;&#160; {toRelativeDate(
											new Date(r.createdAt),
										)}
									</span>
								{/if}
								{#if r.status === "PENDING"}
									<button class="decline" onclick={() => deny(r)}
										>Decline</button
									>
									<button class="approve" onclick={() => approve(r)}
										>Approve</button
									>
								{:else}
