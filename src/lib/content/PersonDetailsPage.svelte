...
let {
	...
	apiPath,
	mediaType,
}: { personId: string | number; apiPath: string; mediaType: MediaTypeE } =
	$props();
...
async function getPerson(id: string | number) {
	return await req.get<PersonDetailsResponse>(`${apiPath}/${id}`);
}

function getMediaId(media: Media) {
	return mediaType === MediaTypeE.olBookAuthor
		? media.ids.olid
		: media.ids.tmdb;
}
...
async function updatePersonCredits() {
	credits = await req.get<PersonCreditsResponse>(
		`${apiPath}/${personId}/credits`,
	);
	credits.credits = credits.credits?.filter(
		(v, i, a) => a.findIndex((t) => getMediaId(t) === getMediaId(v)) === i,
		...
		{#if Object.keys(person).length > 0}
				...
				<div class="details-wrap">
					<div class="details-container">
						{#if person.extPosterPath}
							<PosterImage
								src={mediaType == MediaTypeE.tmdbPerson ? "https://image.tmdb.org/t/p/w500" + person.extPosterPath : person.extPosterPath}
							/>
						{/if}
			...
			{#if credits}
				{#if credits?.credits && credits?.credits?.length > 0}
					...
					<div class="page">
						<PosterList>
							{#each credits.credits as c, i (`${i}-${c.ids.tmdb ?? c.ids.olid}`)}
								<Poster
									media={c}
